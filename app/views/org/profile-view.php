...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div class="flex-1 min-w-0">
            <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
                <?php echo htmlspecialchars($organization['name'] ?? 'Organization Profile'); ?>
            </h2>
        </div>
    ...
    <?php if (!empty($reviewState['hasPending'])): ?>
        ...
        <div class="mb-6 rounded border border-amber-200 bg-amber-50 p-4">
            ...
            <p class="text-sm text-amber-900 mb-3">An admin updated your organization profile. You can accept, decline (revert), or edit details yourself.</p>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        <?php if (!empty($organization['youtube_links'])): ?>
                            <ul class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                <?php
                                $links = preg_split('/[\r\n,]+/', (string) $organization['youtube_links']);

...
<div class="max-w-3xl mx-auto p-6">
    ...
    <form method="post" enctype="multipart/form-data" action="<?= isset($organization) ? '/admin/organizations/edit/' . ($organization['id'] ?? '') : '/admin/organizations/create' ?>">
        ...
        <div class="mb-4 border rounded p-4 bg-gray-50">
            <h2 class="text-sm font-semibold text-gray-700 mb-3">Organization President</h2>
            <div class="mb-3">
                <label class="block text-sm font-medium text-gray-700">President Full Name</label>
                <input name="president_name" value="<?= htmlspecialchars($presidentName ?? '') ?>" class="mt-1 block w-full border rounded px-3 py-2" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">President Email (@mymail.mapua.edu.ph)</label>
                <input type="email" name="president_email" value="<?= htmlspecialchars($presidentEmail ?? '') ?>" class="mt-1 block w-full border rounded px-3 py-2" />
            </div>
        </div>
